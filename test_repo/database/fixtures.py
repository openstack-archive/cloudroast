...
class DBaaSFixture(BaseTestFixture):
    ...
    def setUpClass(cls):
        ...
        #ah_dict = {EngineConfig.SECTION_NAME:
        # {'serializer': 'xml', 'deserializer': 'xml'}}
        #ah_config = cls.dbaas_config.mcp_override(ah_dict)

        #dbaas_atom_hopper_url =
        # ah_config.compute_api.atom_hopper_url + '/database/events'
        # if cls.dbaas_config.atom_hopper_url is not None:
        #     dbaas_atom_hopper_url =
        # cls.dbaas_config.atom_hopper_url + '/database/events'
        #     cls.dbaas_atomhopper_provider =
        # AtomHopperProvider(dbaas_atom_hopper_url, ah_config)
        ...
        if mgmt_username:
            ...
            mgmt_service_url = \
                cls.dbaas_config.mgmt_host + "/" + mgmt_tenant_id
            cls.mgmt_client = \
    ...
    def _test_access(self, users, databases, expected_response="200"):
        ...
        for user in users:
            access = None
            try:
                access = self.reddwarf_client.users.list_access(
    ...
    def _reset_access(self):
        for user in self.users:
            for database in self.databases + self.ghostdbs:
                try:
                    self.reddwarf_client.users.revoke(self.instance.id, user,
                                                      database)
                    self.assertTrue(
                        self.get_last_response_code() in [202, 404])
                except self.failureException:
