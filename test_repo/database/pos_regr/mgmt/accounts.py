...
class test_accounts(DBaaSFixture):
    ...
    def tearDownClass(cls):
        ...
        if test_accounts.instance_id is not None:
            status = testutil.getInstanceStatus(test_accounts.mgmt_dbaas,
                                                instanceId=test_accounts.instance_id)
            if testutil.isInstanceActive(test_accounts.mgmt_dbaas,
    ...
    def test_list_all_active_accounts(self):
        try:
            ...
            self.assertTrue(httpCode == '200',
                            "List all active accounts failed with code %s" % httpCode)
        ...
        for acct in _accounts.accounts:
            ...
            self.fixture_log.debug("\t\tAccount number of instances: %r" % acct["num_instances"])
    ...
    def test_list_account_details(self):
        ...
        try:
            _account_details = self.mgmt_dbaas.accounts.show(selected_acct["id"])
            httpCode = testutil.get_last_response_code(self.mgmt_dbaas)
            self.assertTrue(httpCode == '200',
                            "List account details failed with code %s" % httpCode)
        except Exception as e:
            self.fixture_log.debug("\tException: %r" % e)
            raise
            ...
            if not instance_found and (_account_details.id == self.tenant_id):
                if inst["id"] == self.instance_id:
                    ...
                    self.fixture_log.debug("\t\t\tFound test inst %r for %r" %
                                           (self.instance_id, _account_details.id))
