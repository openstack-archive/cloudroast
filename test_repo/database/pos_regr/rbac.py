...
class RBACTest(DBaaSFixture):
    ...
    def setUpClass(cls):
        ...
        cls.admin_instance_id, time = \
            testutil.create_active_instance(cls.dbaas_admin)
        cls.creator_instance_id, time = \
            testutil.create_active_instance(cls.dbaas_creator)
    ...
    def tearDownClass(cls):
        ...
        status = testutil.getInstanceStatus(cls.dbaas_creator,
        ...
        status = testutil.getInstanceStatus(cls.dbaas_admin,
    ...
    def test_rbac_instance_admin_rights(self):
        ...
        try:
            ...
            user_name, user_password = \
                self.dbaas_admin.root.create(self.admin_instance_id)
            httpCode = testutil.get_last_response_code(self.dbaas_admin)
            self.assertTrue(httpCode ==
                            '200',
            ...
            self.assertTrue(self.dbaas_admin.root.is_root_enabled(
                self.admin_instance_id),
            ...
            self.assertTrue(httpCode ==
                            '202', "DB Resize: Expected http code: %s "
                                   "| Actual http code: %s "
                                   % ('202', httpCode))
            status = testutil.getInstanceStatus(self.dbaas_admin,
                                                self.admin_instance_id)
            self.assertEqual(status, "RESIZE",
                             "Error: instance is not RESIZE, it is: %s"
                             % status)
            testutil.waitForActive(self.dbaas_admin,
            ...
            #resize again
            self.dbaas_admin.instances.resize_instance(
                self.admin_instance_id, final_flavor)
            status = testutil.getInstanceStatus(self.dbaas_admin,
                                                self.admin_instance_id)
            self.assertEqual(status, "RESIZE",
                             "Error: instance is not RESIZE, it is: %s"
    ...
    def test_rbac_instance_creator_rights(self):
        ...
        try:
            ...
            httpCode = testutil.get_last_response_code(self.dbaas_creator)
            self.assertTrue(httpCode == '202',
                            "DB Resize instance: Expected http code: %s "
                            "| Actual http code: %s "
                            % ('202', httpCode))
            status = testutil.getInstanceStatus(self.dbaas_creator,
                                                self.creator_instance_id)
            self.assertEqual(status, "RESIZE",
                             "Error: instance is not RESIZE, it is: %s"
                             % status)
            testutil.waitForActive(self.dbaas_creator,
                                   instanceId=self.creator_instance_id)

            #get the new flavorId
            flavorId = self.dbaas_creator.instances.get(
                self.creator_instance_id).flavor["id"]
            self.assertEqual(str(next_flavor), flavorId,
                             "Error: Flavors do not match: %s != %s"
                             % (next_flavor, flavorId))
            #resize again
            self.dbaas_creator.instances.resize_instance(
                self.creator_instance_id, final_flavor)
            status = testutil.getInstanceStatus(self.dbaas_creator,
                                                self.creator_instance_id)
            self.assertEqual(status, "RESIZE",
                             "Error: instance is not RESIZE, it is: %s"
            ...
            httpCode = testutil.get_last_response_code(self.dbaas_creator)
            self.assertTrue(httpCode == '202',
                            "DB Resize volume: Expected http code: %s "
                            "| Actual http code: %s "
                            % ('202', httpCode))
            status = testutil.getInstanceStatus(self.dbaas_creator,
                                                self.creator_instance_id)
            ...
            #Get the new volume size
            volume_size = self.dbaas_creator.instances.get(
                self.creator_instance_id).volume["size"]
            self.assertEqual(resize_vol, volume_size,
                             "Error: Flavors do not match: %s != %s"
                             % (resize_vol, volume_size))
