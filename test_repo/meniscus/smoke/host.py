...
class TestHost(HostFixture):
    ...
    def test_create_host(self):
        result = self._create_new_host(ip_v4=self.tenant_config.ip_address_v4,
                                       ip_v6=self.tenant_config.ip_address_v6,
                                       profile_id=self.profile_id)
        self.assertEqual(result['request'].status_code, 201)

    def test_delete_host(self):
        result = self._create_new_host()
        response = self._delete_host(result['host_id'])

        self.assertEqual(response.status_code, 200)

    def test_update_host(self):
        host_result = self._create_new_host()
    ...
    def test_get_all_hosts(self):
        ...
        host1_result = self._create_new_host(hostname=first_hostname)
        host2_result = self._create_new_host(hostname=second_hostname)
        ...
        msg = 'Couldn\'t find host id and name in retrieved host list'
        self.assertIn(True, [host.id == host1_id and
                             host.hostname == first_hostname
                             for host in hosts], msg)
        self.assertIn(True, [host.id == host2_id and
                             host.hostname == second_hostname
                             for host in hosts], msg)
