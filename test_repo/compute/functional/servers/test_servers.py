...
class ServersTest(ComputeFixture):
    ...
    def setUpClass(cls):
        ...
        cls.meta = {'meta_key_1': 'meta_value_1',
    ...
    def test_create_server_with_image_and_flavor_self_link(self):
        ...
        # Verify that the image Id of the image ref link which is used
        # to create server is same as the image id of the created server
        self.assertEqual(
            get_server_info.image.id,
            self._parse_link_to_retrieve_id(image_self_link),
            msg="The image does not match the image used during creation")

        # Verify that the flavor Id of the flavor ref link which is used
        # to create server is same as the flavor id of the created server
        self.assertEqual(
            get_server_info.flavor.id,
            self._parse_link_to_retrieve_id(flavor_self_link),
            msg="The flavor does not match the flavor used during creation")
    ...
    def test_create_server_with_image_and_flavor_bookmark_link(self):
        ...
        create_server_response = self.servers_client.create_server(
            name, image_bookmark_link, flavor_bookmark_link)
        created_server = create_server_response.entity
        ...
        self.assertEqual(
            get_server_info.image.id,
            self._parse_link_to_retrieve_id(image_bookmark_link),
            msg="The image does not match the image used during creation")
        self.assertEqual(
            get_server_info.flavor.id,
            self._parse_link_to_retrieve_id(flavor_bookmark_link),
            msg="The flavor does not match the flavor used during creation")
    ...
    def test_update_server_using_server_self_link(self):
        ...
        # Need to ensure there is at least one second gap between creating and
        ...
        updated_server_response = self.servers_client.update_server(
            server_id, name, accessIPv4=self.accessIPv4,
        ...
        self.assertEqual(stored_name, server.name,
                         msg="The name was not updated")
        self.assertEqual(self.accessIPv4, server.accessIPv4,
                         msg="AccessIPv4 was not updated")
        self.assertEqual(self.accessIPv6, server.accessIPv6,
                         msg="AccessIPv6 was not updated")
        self.assertEqual(active_server.created, server.created,
                         msg="The creation date was updated")
        self.assertTrue(
            active_server.updated != server.updated,
            msg='Updated time for server %s did not change '
                'after a modification to the server.' % server.id)
    ...
    def test_update_server_using_server_bookmark_link(self):
        ...
        # Verify the name and access ips of the server have changed
        server = self.servers_client.get_server(updated_server.id)
        self.assertEqual(stored_name, server.entity.name,
                         msg="The name was not updated")
        self.assertEqual(self.accessIPv4, server.entity.accessIPv4,
                         msg="AccessIPv4 was not updated")
        self.assertEqual(self.accessIPv6, server.entity.accessIPv6,
                         msg="AccessIPv6 was not updated")
        self.assertEqual(active_server.created, server.entity.created,
                         msg="The creation date was updated")
        self.assertTrue(
            active_server.updated != server.entity.updated,
            msg='Updated time for server %s did not change '
                'after a modification to the server.' % updated_server.id)

        self.servers_client.delete_server(server.entity.id)
