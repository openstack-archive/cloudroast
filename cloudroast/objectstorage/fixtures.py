...
def get_pipeline(client, endpoint):
    ...
    if not response.ok:
        raise Exception('Could not retrieve pipeline from swift: {0}: {1}'
                        .format(str(response.status_code), response.content))

    try:
        middleware = json.loads(response.content)
    except ValueError:
        raise Exception('Invalid JSON recieved from Swift MiddlewareCheck.')

    return middleware['swift_proxy']['pipeline']
