...
class CreateServerTest(ComputeFixture):
    ...
    def test_config_drive_openstack_metadata(self):
        ...
        self.assertEqual(self.files[0].get('path'), openstack_meta.files.path,
                         msg=message.format('path do not match',
                                            self.key.name,
                                            openstack_meta.public_keys.name))
        ...
        self.assertEqual(self.key.name, openstack_meta.public_keys.name,
                         msg=message.format('key name do not match',
                                            self.key.name,
                                            openstack_meta.public_keys.name))
...
#                        msg="local_ipv4 was not set in the response")
        ...
        self.assertEqual(self.key.name, ec_meta.public_keys.name,
                         msg=message.format('key name', self.key.name,
                                            ec_meta.public_keys.name))
