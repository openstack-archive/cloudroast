...
class CloudInitUpstartTest(ComputeFixture):

    @classmethod
    def setUpClass(cls):
        super(CloudInitUpstartTest, cls).setUpClass()
        init_st = cls.config_drive_behaviors.read_cloud_init_from_config_drive(
            cls.cloud_init_config.upstart_script)
    ...
    def test_cloud_config_input_format(self):
        ...
        instanse_user_data = remote_client.get_file_details(
            filepath=self.user_data_filepath)
        self.assertEqual(instanse_user_data.content,
                         ...
                         msg=message.format('Configuration do not match',
                                            ...
                                            self.user_data_contents))
        include_file = self.config_drive_behaviors.status_of_manage_etc_hosts( 
            server=self.server, servers_config=self.servers_config,
            key=self.key.private_key)
        include_file_present = remote_client.is_file_present(
            filepath="/var/lib/cloud/instances/{0}/obj.pkl".format(
                self.server.id))
        ...
        self.assertIn('HELLO WORLD: $UPSTART_JOB', hosts, 
