...
class CloudInitMimeGzipTest(ComputeFixture):
    ...
    def test_gzip_mime_format(self):
        ...
        dir_cloud_config_present = self.config_drive_behaviors.status_of_manage_etc_hosts( 
            server=self.server, servers_config=self.servers_config,
            key=self.key.private_key)
        ...
        try:
            ...
            pass    
        self.assertIn('mynode', hosts, msg="Managed etc hosts was enabled from script")
        # Verification of user data script of mime
        try:
            users = remote_client.get_file_details(
                                                   "/etc/passwd").content
        except Exception: 
            pass
