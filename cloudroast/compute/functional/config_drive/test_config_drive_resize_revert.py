...
class ResizeServerUpRevertTests(ComputeFixture):
    ...
    def setUpClass(cls):
        ...
        cls.user_data_before = remote_client.get_file_details(
            file_path=cls.user_data_filepath).content
        cls.kb_size_before = remote_client.get_directory_details(
            cls.config_drive_config.base_path_to_mount)
    ...
    def test_openstack_metadata(self):
        ...
        self.assertEqual(
            ...
            msg="Expected {0} but received {1}".format(
                self.openstack_meta_before_resize,
                openstack_meta_after_resize))
        self.assertIsNotNone(openstack_meta_after_resize.availability_zone)
        self.assertIsNotNone(openstack_meta_after_resize.hostname)
        self.assertIsNotNone(openstack_meta_after_resize.launch_index)
        self.assertEqual(
