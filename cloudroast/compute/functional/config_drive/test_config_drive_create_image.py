...
class CreateImageConfigDriveTest(CreateServerFixture):
    ...
    def setUpClass(cls):
        ...
        cls.user_data_before_image = remote_client.get_file_details(
    ...
    def test_can_create_server_from_image(self):
        """Verify that a new server can be created from the image
           And that the config drive is mounted on the new image"""
    ...
    def test_create_server_openstack_metadata(self):
        ...
        # Verify that the metadata before the build matchs the
        # metadata after the build
