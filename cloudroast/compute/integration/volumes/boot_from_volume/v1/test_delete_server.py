...
class DeleteVolumeServersTest(object):
    ...
    def test_volume_not_deleted_after_server_deletion(self):
        ...
        # Get the Volume Information and check if it match with the provided one
        volumes = self.volume_attachments_client.get_server_volume_attachments(
            server.id).entity
        self.assertEqual(self.volume.id_, volumes[0].volume_id,
                         msg=message.format('volume id', self.volume.id_,
                                            volumes[0].volume_id))
