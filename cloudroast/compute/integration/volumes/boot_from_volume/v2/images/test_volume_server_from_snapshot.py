...
class CreateVolumeServerfromSnapshotTest(ServerFromVolumeV2Fixture):
    ...
    def setUpClass(cls):
        ...
        cls.addClassCleanup(
            cls.volumes.behaviors.delete_snapshot_confirmed, cls.snapshot.id_)

    @tags(type='smoke', net='no')
    def test_create_volume_server_from_regular_image_snapshot(self):
    ...
    @tags(type='smoke', net='no')
    def test_create_volume_server_from_regular_volume_snapshot(self):
        """Verify the response code for a create image request is correct."""
        # Creating block device with snapshot data inside
        self.block_data = self.server_behaviors.create_block_device_mapping_v2(
            boot_index=0, uuid=self.snapshot.id_,
            volume_size=self.volume_size,
