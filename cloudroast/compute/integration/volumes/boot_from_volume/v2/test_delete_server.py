...
class DeleteVolumeServersTest(object):
    ...
    def test_delete_volume_true(self):
        """Verify Automated on default deletion worked"""
        with self.assertRaises(ItemNotFound):
            self.volumes.client.delete_volume(self.volume_id)

    @tags(type='smoke', net='no')
    def test_volume_is_preserved(self):
        """Verify Automated on default preserve succeed"""
        ...
        volume_resp = self.client.get_volume_info(volume_id)
        self.assertEqual(volume_resp.status_code, 202)
        # Clean the Volume
        result = self.volumes.behaviors.delete_volume_confirmed(volume_id)
