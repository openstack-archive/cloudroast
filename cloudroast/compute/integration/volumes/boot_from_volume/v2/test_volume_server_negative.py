...
class CreateVolumeServerNegativeTest(ServerFromVolumeV2Fixture):
    ...
    def test_delete_on_terminate_invalid(self):
        """Verify default behaviour works as expected with delete invalid"""
    ...
    def test_type_invalid(self):
        """Verify default behaviour is working when type invalid"""
        # Creating block device with snapshot data inside
        self.block_data = self.server_behaviors.create_block_device_mapping_v2(
            boot_index=0, uuid=self.image_ref,
            volume_size=self.volume_size,
            source_type='', destination_type='volume',
    ...
    @tags(type='smoke', net='no')
    def test_boot_from_concurent_sources(self):
        """Verify default behaviour is booting from image"""
        # Creating block device with snapshot data inside
        self.block_data = self.server_behaviors.create_block_device_mapping_v2(
            boot_index=0, uuid=self.image_ref,
            volume_size=self.volume_size,
    ...
    def test_flavor_invalid(self):
        """Verify boot is failing with invalid flavor"""
        # Creating block device with snapshot data inside
        self.block_data = self.server_behaviors.create_block_device_mapping_v2(
            boot_index=0, uuid=self.image_ref,
            volume_size=self.volume_size,
