...
class GlanceScrubberTest(ObjectstorageIntegrationFixture):
    ...
    def test_chunks_present_after_image_create(self):
        ...
        message = "Expected {input} to be >= {expectation}, was {reality}"

        chunks = self._get_image_chunks(self.prefix)

        self.assertGreaterEqual(chunks, 1, msg=message.format(
            input="Image Chunks", expectation=1, reality=chunks))
    ...
    def test_chunks_deleted_after_image_delete(self):
        ...
        self.assertLessEqual(chunks, 0, msg=message.format(input='Image Chunks',
                             expectation=0,
                             reality=chunks))
