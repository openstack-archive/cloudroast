...
class ServerRHELActivationTests(object):
    ...
    def test_check_rhel_activation(self):
        ...
        if not self.server_list:
            assert False, "No Servers created for the activation test"
...
class ServerFromImageRHELActivationTests(ServerFromImageFixture,
    ...
    def setUpClass(cls):
        ...
        cls.key_response = cls.keypairs_client.create_keypair(rand_name("key"))

        if cls.key_response.entity is None:
                raise Exception(
                    "Response entity of create key was not set. "
                    "Response was: {0}".format(cls.key_response.content))

        cls.key = cls.key_response.entity
