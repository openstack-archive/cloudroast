...
class ConfigDriveFilesTest(ComputeFixture):
    ...
    def test_config_drive_network_metadata_networks(self):
        ...
        for ip in expected_addresses:
            self.assertIn(ip, config_drive_instance_ips,
                          "The expected public ip: %s was not found in "
                          "the config drive ips" % ip)

    @tags(type='smoke', net='yes')
    def test_config_drive_network_metadata_file_structure(self):
