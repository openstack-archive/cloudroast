...
class ServersTest(ComputeFixture):
    ...
    def test_create_server_with_admin_password(self):
        """
        Creates a server with an admin password passed in which is expecting to be set as the root user's password

        This will set the server that is created with the root password of oldslice129690TuG72Bgj2.  Calls cloudcafe's
        server behaviors get remote instance client with the password to validate that it can authenticate wth password
        The following assertions occur.
            - Defined password is the same as the admin password returned on the server
            - Remote client returned get_remote_instance_client has a true value for can_authenticate
        """
    ...
    def test_update_server(self):
        """
        Creates a server and then updates the name, ipv4 and ipv6 values then calls get server to validate

        Will call the create_active_server from server behaviors, then call update_server from cloudcafe's server_client
        passing in the newly created server's id, new random name, ipv4 address 192.168.32.16 and ipv6 address
        3ffe:1900:4545:3:200:f8ff:fe21:67cf and then waiting for the server to make it to active status.
        Calling get_server to validate the updates occured correctly
        The following assertions occur.
