...
def _pinger(ip, delta, connection_timeout):
    """
...
class LiveMigratation(ComputeAdminFixture):
    ...
    def test_connectivity_during_live_migrate_server(self):
        ...
        if self.delta.empty():
            ...
            assert int(self.delta.get()) < self.ping_timeout, "It took {delta},"
            " to have connectivity after live migrate, Expected timeout was"
            " {timeout}".format(delta=self.delta.get(), timeout=self.ping_timeout)
