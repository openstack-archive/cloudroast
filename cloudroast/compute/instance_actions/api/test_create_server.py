...
class ServerFromImageCreateServerTests(ServerFromImageFixture,
    ...
    def test_created_server_primary_disk(self):
        """
        Verify the size of the virtual or physical primary disk matches
        the size set by the flavor
        """
        remote_client = self.server_behaviors.get_remote_instance_client(
            self.server, self.servers_config, key=self.key.private_key)
        ...
        if hypervisor == ComputeHypervisors.IRONIC:
            # For on metal the tolerance should be not more than 20%
            delta = self.images_config.delta_image_size
            self.assertAlmostEqual(
                disk_size,
                self.flavor.disk,
                msg="Expected disk to be {exp}+/-{delta} GB, was {act} GB".format(
                    exp=self.flavor.disk, delta=delta, act=disk_size), delta=delta)
        else:
