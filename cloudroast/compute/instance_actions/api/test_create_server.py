...
class CreateServerTest(object):
    ...
    def test_create_server_admin_password(self):
        ...
        Validate that the server created during test set up has a admin password
        value.
    ...
    def test_created_server_fields(self):
        ...
        The name of the server created during test set up should match the
        server name value generated during test set up. The flavor id of the
        server created during test set up should match the flavor id identified
        in test configuration. The 'created' and 'updated' values of the server
        created during test set up should not be None and the 'updated' value
        should be greater than the 'created' value.
    ...
    def test_created_server_ephemeral_disk(self):
        """
        Server's ephemeral disk size should equal the flavor's disk size

        The ephemeral disk size of the server created during test set up should
        be equal to the ephemeral disk size of the flavor from test
        configuration. If the flavor's ephemeral disk size is 0, the test will
        return and end. If the flavor's ephemeral disk size is not 0, get a
        remote client for the server created during test set up. Use the remote
        client to get the a list of all disks on the server. Remove the primary
        disk from the list of disks. Verify that the remaining disks in the
        disk list are equal in size to the ephemeral disk size of the flavor
        from test configuration. Format each disk in the list of disks and mount
        them to the server.
        """
