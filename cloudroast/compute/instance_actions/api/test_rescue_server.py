...
from cloudroast.compute.fixtures import requires_extension
...
class ServerRescueTests(object):
    ...
    def test_rescue_with_image_change_server_test(self):
        """
        Verify that a server can enter and exit rescue mode when image
        supplied to the rescue request this test will execute only if the
        extension is enabled this would be determined automatically
        """
        server = self.create_server(key_name=self.key.name)
        remote_client = self.server_behaviors.get_remote_instance_client(
            server, self.servers_config, key=self.key.private_key)
        distro_before_rebuild = remote_client.get_distribution_and_version()
        ...
        if (distro_before_rebuild and
                distro_after_rebuild and
                self.image_ref != self.image_ref_alt):
            self.assertNotEqual(distro_before_rebuild,
                                distro_after_rebuild)
        else:
            self.assertEqual(distro_before_rebuild, distro_after_rebuild)
