...
class RebuildServerTests(object):
    ...
    def test_verify_rebuild_server_response(self):
        ...
        Validate that the server rebuilt during test setup has a the expected
        values for the server name, image id, flavor id, server id, links,
        metadata, server created time and network addresses are equal to those
        set during initial server creation or used during the rebuild request.
        Validate that the server updated value has changed.
    ...
    def test_rebuilt_server_addresses(self):
        ...
        The following assertions occur:
            - For each network in the expected networks value from test
              ...
              validate that there is an IPv4 for the corresponding network on
              the rebuilt server. If the key is not found, check there there is
              not an IPv4 for the corresponding network on the rebuilt server.
            - For each network in the expected networks value from test
              configuration, if the key 'v6' is in the dictionary of addresses
              validate that there is an IPv6 for the corresponding network on
              the rebuilt server. If the key is not found, check there there is
              not an IPv6 for the corresponding network on the rebuilt server.
        """
        rebuilt_server = self.rebuilt_server_response.entity
    ...
    def test_address_not_changed_on_rebuild(self):
        ...
        Validate that the network addresses of a rebuilt server are equal to the
        network addresses of the server prior to rebuild
    ...
    def test_rebuilt_server_ephemeral_disk(self):
        ...
        0, get a remote client for the server rebuilt during test set up. Use
    ...
    def test_rebuilt_server_instance_actions(self):
        ...
        Filter the list so that only the actions 'rebuild' remain. Validate that
    ...
    def test_distro_after_rebuild(self):
        ...
        Get a remote instance client for the server rebuilt during test set up.
        Use the remote client to get the distribution and version of the server.
        If there is a value for the server's distro before rebuild, the test
        was successful in getting the current distro, and the image id and alt
        image id in test configuration are different validate that the distro
        before rebuild and distro after rebuild are different. Otherwise
...
class RebuildBaseFixture(object):
    ...
    def rebuild_and_await(cls):
        ...
        The follow data is generated:
            - a metakey dictionary containing:
                {'key': 'value'}
            ...
            - If default file injection is enabled, a file path for a file named
             'rebuild.txt' with the contents 'Test server rebuild.'
