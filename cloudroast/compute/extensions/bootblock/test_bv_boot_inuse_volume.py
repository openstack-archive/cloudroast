...
class BootInUseVolume(ComputeFixture):
    ...
    def setUpClass(cls):
        ...
        cls.err_msg = ""
        cls.boot_from_block = {'volume_id': cls.server.volume_id,
    ...
    def test_for_inuse_volume_should_not_be_used(self):
        try:
            self.response = self.server_behaviors.create_active_server(
                boot_from_block=self.boot_from_block)
        except Exception as self.err_msg:
            self.assertEqual(str(self.err_msg).find("failed"), True,
                             self.err_msg)
        else:
            self.server = self.response.entity
            self.resources.add(self.server.id,
                               self.servers_client.delete_server)
        self.fail("Test case should have failed because volume was inuse")
