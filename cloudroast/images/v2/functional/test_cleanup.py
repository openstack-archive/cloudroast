...
class TestImagesCleanUp(ObjectStorageIntegrationFixture):
    ...
    def test_images_cleanup(self):
        ...
        print '{0} server(s)'.format(len(servers))
        ...
        print '{0} container(s)'.format(len(containers))
        ...
        print '{0} image(s) for primary user'.format(len(images))
        for image in images:
        ...
        count = 1
        owner = self.alt_tenant_id
        images = self.alt_images_behavior.list_images_pagination(owner=owner)
        print '{0} image(s) for alternate user'.format(len(images))
        for image in images:
        ...
        count = 1
        owner = self.third_tenant_id
        images = self.third_images_behavior.list_images_pagination(owner=owner)
        print '{0} image(s) for third user'.format(len(images))
        for image in images:
