...
class ImageLifeCycleTenantPermissionsTest(ImagesFixture):
    ...
    def test_tenant_permissions_on_image_life_cycle(self):
        ...
        2. Verify Tenant 1 can access the image directly
        3. Verify that Tenant 1 can see the image in their list
        4. Verify Tenant 2 (as a non admin and non member of the image)
        cannot access the image directly
        5. Verify Tenant 2 cannot see the image in their list
        6. Verify Tenant 2 cannot update the image
        7. Verify Tenant 2 cannot delete the image
        8. Add Tenant 2 as a member of the image
        9. Verify Tenant 2 can now access the image directly
        10. Verify Tenant 2 still cannot update the image
        11. Verify Tenant 2 still cannot to delete the image
        12. Verify that Tenant 2 still cannot see the image in their list
        ...
        15. Verify that Tenant 2 still cannot update the image
        16. Verify that Tenant 2 still cannot delete the image
        """

        Tenant2_id = self.alt_access_data.token.tenant.id_
        ...
        response = self.images_client.add_member(
            image_id=image.id_, member_id=Tenant2_id)
        self.assertEqual(response.status_code, 200)
