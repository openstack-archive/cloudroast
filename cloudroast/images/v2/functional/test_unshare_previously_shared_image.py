...
class UnsharePreviouslySharedImageTest(ImagesFixture):
    ...
    def test_unshare_previously_shared_image(self):
        """@summary: Unshare previously shared image between two users A and B

        1. Register an image as user A
        2. Verify that user B cannot access the image
        3. Add user B as a member of the image
        4. Verify that user B can now access the image
        5. Update user B membership status to 'accepted' for image
        6. Verify that the image belongs to user B list of images
        7. Remove user B as a member of the image
        8. Verify that the list of members for the image is empty
        9. Verify that the image does not belong to user B list of images
        """
