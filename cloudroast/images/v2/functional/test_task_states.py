...
class TestTaskStates(ImagesFixture):
    ...
    def test_import_task_states_failure(self):
        ...
        6) Verify that the status is 'pending'
        7) Get task, verify that the status changes from 'pending' to
        'processing'
        8) Delete the file being imported
        9) Get task, verify that the status changes from 'processing' to
        'failure'
        10) Verify that the task's properties appear correctly
        11) Verify that a result property is not returned
        12) Verify that a message property with the proper error message is
        returned
        """
        ...
        if (task.message != 'Error: Swift could not find object '
                '{0}.'.format(object_name)):
            errors.append(self.error_msg.format(
                'message', 'Error: Swift could not find object '
                '{0}.'.format(object_name), task.message))
    ...
    def test_export_task_states_failure(self):
        ...
        4) Get task, verify that the status changes from 'pending' to
        'processing'
        5) Delete the image being exported
        ...
        if task.result is not None:
            errors.append(self.error_msg.format(
                'export_location', None, task.result))
        if (task.message != 'container {0} does not '
                'exist'.format(container_name)):
            errors.append(self.error_msg.format(
                'message', 'container {0} does not '
                'exist'.format(container_name), task.message))
