...
class PatchImageTest(ImagesV2Fixture):
    ...
    def test_update_image_name(self):
        ...
        self.assertEqual(
            ...
            self.msg.format('status_code', 200, response.status_code))
        self.assertEqual(
            updated_image.name, updated_image_name,
            self.msg.format('name', updated_image_name, updated_image.name))
        self.assertEqual(
            updated_image.id_, image_id,
