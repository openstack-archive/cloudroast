...
class VisibilityOfImagesSharedBetweenUsersTest(ImagesFixture):
    ...
    def test_visibility_of_images_shared_between_users(self):
        ...
        1) Register image (image_share_accept) as tenant
        2) Register another image (image_share_reject) as tenant
        3) Register another image (image_share_pending) as tenant
        4) Register an image (alt_user_image) as alternative tenant
        5) Register an image (third_user_image) as third tenant
