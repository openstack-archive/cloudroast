...
class ImageVisibilityLifeCycleTest(ImagesFixture):
    ...
    def test_image_visibility_life_cycle(self):
        ...
        1. Register an image as tenant A
        2. Verify that image's visibility is private (set by default)
        3. Verify that tenant A can access the image directly
        4. Verify that tenant B, a non admin and non member, cannot access the
        image directly
        5. Add tenant B as a member of the image
        6. Verify that tenant B can now access the image
        7. Verify that tenant B cannot update image's visibility
        8. Verify that tenant A cannot remove image visibility
        9. Verify that tenant A cannot update image's visibility to an invalid
        state
        10. Update image visibility to 'public', as tenant A
        ...
        response = self.admin_images_client.add_member(
            image_id=image.id_, member_id=tenant_B_id)
        self.assertEqual(response.status_code, 200)
        ...
        response = self.admin_images_client.add_member(
            image_id=image.id_, member_id=tenant_C_id)
        self.assertEqual(response.status_code, 403)
