    ...
    def test_update_image_property_quota_limit(self):
        ...
        for prop in additional_props:
            if hasattr(image, prop):
                quota_limit -= 1
        number_of_props = len(image.additional_properties)

        while number_of_props != quota_limit:
            response = self.images_client.update_image(
                image.id_, add={rand_name('new_prop'):
                                rand_name('new_prop_value')})
            self.assertEqual(response.status_code, 200)
            number_of_props = len(response.entity.additional_properties)
        ...
        self.assertEqual(response.status_code, 200)
        get_image = response.entity
        self.assertEqual(len(get_image.additional_properties), quota_limit)
