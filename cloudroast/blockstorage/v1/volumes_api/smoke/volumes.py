...
from cloudroast.blockstorage.v1.fixtures import \
    BlockStorageTestFixture, BlockStorageDatasets


@DataDrivenFixture
class VolumeActions(BlockStorageTestFixture):
    ...
    def ddtest_create_minimum_size_default_volume(
        ...
        #Setup
        size = self.volumes_api_config.min_volume_size
        ...
        availability_zone = self.blockstorage_config.region
        ...
        volume = resp.entity
        self.addCleanup(
            self.volumes_behaviors.delete_volume_confirmed, volume.id_)
        ...
        #Works on rackspace cloud only
        self.assertEquals(volume.volume_type, volume_type_name)

    @data_driven_test(BlockStorageDatasets.volume_types())
    @tags('smoke')
    def ddtest_get_volume_info(self, volume_type_name, volume_type_id):
        volume = self.new_volume(vol_type=volume_type_id)
        resp = self.volumes_client.get_volume_info(volume.id_)
    ...
    def ddtest_list_volume_details(self, volume_type_name, volume_type_id):
        volume = self.new_volume(vol_type=volume_type_id)
        resp = self.volumes_client.list_all_volumes_info()
        self.assertExactResponseStatus(
            resp, 200, msg='Get volume list call failed')
        self.assertResponseIsDeserialized(resp)
        volume_list = resp.entity
    ...
    def ddtest_delete_volume(self, volume_type_name, volume_type_id):
        volume = self.new_volume(vol_type=volume_type_id)
        result = self.volumes_behaviors.delete_volume_confirmed(volume.id_)
