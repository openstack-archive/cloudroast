...
class CBSVolumeCloneTests(DataDrivenVolumesTestFixture):
    ...
    def ddtest_create_exact_clone_of_existing_volume_and_verify_attributes(
        ...
        # TODO: Make sure that a cinder clone is supposed to inherit it's
        # source volume's user-defined metadata.
    ...
    def ddtest_create_larger_clone_of_volume(
            self, volume_type_name, volume_type_id):
        # SETUP
        # Create a volume A, wait for volume to become available
        # Create a volume B with source_volid = A.id and size = A.size + 1
        # TEST:
        # Expect create call for B to return a 200
        # Expect B to reach the 'available' status
