...
class ReactivateRegisteredImage(ImagesFixture):

    @classmethod
    def setUpClass(cls):
        super(ReactivateRegisteredImage, cls).setUpClass()
        ...
        cls.images_admin.client.deactivate_image(cls.registered_image.id_)

        cls.alt_registered_image = registered_images.pop()
        cls.images.client.store_image_file(
            cls.alt_registered_image.id_, cls.images.config.test_file)
        cls.images_admin.client.deactivate_image(cls.alt_registered_image.id_)
    ...
    def tearDownClass(cls):
        ...
        super(ReactivateRegisteredImage, cls).tearDownClass()

    def test_reactivate_registered_image(self):
        """
        @summary: Reactivate a registered image

        1) Reactivate a registered image
    ...
    @unittest.skip('Launchpad bug #1447322')
    def test_reactivate_queued_image(self):
        """
        @summary: Reactivate a queued image

        1) Reactivate a queued image
    ...
    def test_reactivate_registered_image_using_non_admin(self):
        """
        @summary: Reactivate a registered image as non-admin

        1) Reactivate a registered image as non-admin
    ...
    def test_list_reactivated_registered_image(self):
        ...
        3) Verify that the reactivated registered image is in the returned list
        of images
        """
