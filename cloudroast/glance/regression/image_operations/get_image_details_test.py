...
class GetImageDetails(ImagesFixture):
    ...
    def setUpClass(cls):
        ...
        cls.image_creation_time_in_sec = calendar.timegm(time.gmtime())
    ...
    def _validate_image(self, get_image):
        ...
        created_at_delta = self.images.behaviors.get_time_delta(
            self.image_creation_time_in_sec, get_image.created_at)
        updated_at_delta = self.images.behaviors.get_time_delta(
            self.image_creation_time_in_sec, get_image.updated_at)
        ...
        if get_image.size > self.images.config.size:
            errors.append(Messages.PROPERTY_MSG.format(
                'size', 'Around {0}'.format(self.images.config.size),
                get_image.size))
