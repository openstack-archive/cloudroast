...
class NetTypes(object):
    ...
    # REASON: I keep associating inet with internet orlinux's
...
class MeshedNetworkFixture(NetworkingComputeFixture):
    ...
    def setUpClass(cls):
        ...
        # NOTE: All ordered dicts use server_id as key
        cls.proxy_mgrs = OrderedDict()
        cls.servers = OrderedDict()
        cls.net_personas = OrderedDict()
    ...
    def setUp(self):
        ...
        for svr_num in xrange(self.NUM_TO_BUILD):
            try:
                svr_id = self.servers.keys()[svr_num]
            except IndexError:
                server = None
            else:
                server = self.servers.get(svr_id, None)
    ...
    def delete_registered_server(self, server_id):
        ...
        del self.net_personas[server_id]
        del self.proxy_mgrs[server_id]
    ...
    def _verify_action_connectivity(self, method, **kwargs):
        ...
        for net_type in NetTypes.ALL_NETWORK_TYPES:
            svr_conn, results_table = api(
                personas=self.net_personas, proxies=self.proxy_mgrs,
                net_type=net_type, **kwargs)
    ...
    def _build_and_register_iso_net_server(self, svr_id_num, iso_network):
        ...
        self.servers[server_resp.entity.id] = server
        ...
        self.net_personas[server.id] = ServerPersona(
            server=server, network=iso_network, inet_fix_ipv4_count=1,
            inet=True)
        self.fixture_log.debug(self.net_personas[server.id])
        ...
        self.proxy_mgrs[server.id] = proxy

        return server.id

    def ssh_net_mesh(self, personas, proxies, net_type, username=None,
                     password=None):
        """
        SSH from each host to every other host contained in the personas.

        :param personas: Dictionary of personas - each persona representing a
            server in the network mesh.
        :param proxies: Dictionary of proxies - each proxy serves a server
            in the network mesh.
        ...
        return self._action_on_net_mesh(
            personas=personas, proxies=proxies, net_type=net_type,
            action=action, username=username, password=password)

    def ping_net_mesh(self, personas, proxies, net_type, ip_version=4,
                      ping_count=DEFAULT_PING_COUNT, threshold=1):
        """
        ...
        return self._action_on_net_mesh(
            personas=personas, proxies=proxies, net_type=net_type,
            action=action, threshold=threshold, ping_count=ping_count,
            ip_version=ip_version)

    def _action_on_net_mesh(self, personas, proxies, net_type, action,
                            ip_version=4, **kwargs):
        """
        ...
        address_pool = [getattr(pers, network_attr)[0] for pers in
                        personas.values() if getattr(pers, network_attr)[0] is
                        not None]

        # Get list of server ids for each server in mesh...
        svr_ids = [getattr(getattr(p, 'server'), 'id') for p in
                   personas.values()]
        ...
        for index in xrange(len(address_pool)):
            ...
            for svr_idx in xrange(len(address_pool)):
                ...
                action_api_args = self._build_proxy_api_args(
                    action=action, svr_idx=svr_idx, address_pool=address_pool,
                    **kwargs)
                ...
                if svr_idx != index:
                    ...
                    # the table
                        ...
                        msg = result_msg.format(
                            src=src_ip, dest=address_pool[svr_idx],
                            result=result)
    ...
    def _build_proxy_api_args(self, action, svr_idx, address_pool, **kwargs):
        """
        Builds basic proxy args (tightly coupled with NetworkProxy class)

        :param action: ping or ssh
        :param svr_idx: index of server  (in proxy/server lists)
        ...
        args_dict = {'target_ip': address_pool[svr_idx]}
