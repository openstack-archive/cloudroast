    ...
    def setUp(self):
        ...
        if not connectivity:
            self.assertClassSetupFailure(
                'Servers do not have full connectivity. See logs for '
                'details.\nResults:\n{0}\n{1}\n{2}'.format(
                    self.last_connectivity_check[NetTypes.PUBLIC_NET],
                    self.last_connectivity_check[NetTypes.SVC_NET],
                    self.last_connectivity_check[NetTypes.ISO_NET]))
