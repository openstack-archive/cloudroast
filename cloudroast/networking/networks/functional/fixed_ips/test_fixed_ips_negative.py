...
class TestFixedIPsMultiple(NetworkingComputeFixture):
    ...
    def _remove_all_ips(self, server, port_type):
        """
        ...
        if port_type == self.pub_net_id:
            server_persona = ServerPersona(
                ...
                pnet_fix_ipv4_count=self.FIXED_IPS_TO_ADD+1,
                snet_fix_ipv4_count=1,
                inet_fix_ipv4_count=1,
                network=self.network, keypair=self.keypair,
                ssh_username='root')
        ...
        if port_type == self.pri_net_id:
            server_persona = ServerPersona(
                ...
                snet_fix_ipv4_count=self.FIXED_IPS_TO_ADD+1,
                inet_fix_ipv4_count=1,
                network=self.network, keypair=self.keypair,
                ssh_username='root')
        ...
        if port_type == self.iso_net_id:
            server_persona = ServerPersona(
                ...
                inet_fix_ipv4_count=self.FIXED_IPS_TO_ADD+1,
                network=self.network, keypair=self.keypair,
                ssh_username='root')
