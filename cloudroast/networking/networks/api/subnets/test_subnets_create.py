...
class SubnetCreateTest(NetworkingAPIFixture):
    ...
    def test_ipv4_subnet_create_w_multiple_params(self):
        ...
        cidr = self.subnets.behaviors.create_ipv4_cidr()
        self.expected_subnet.cidr = cidr
        self.expected_subnet.allocation_pools = self.get_allocation_pools_data(
            cidr=self.expected_subnet.cidr, start_increment=3, ip_range=20,
            interval=10, n=3)
        ...
        nexthop = self.subnets.behaviors.get_ip(self.expected_subnet.cidr)
        host_route = dict(destination='10.0.3.0/24', nexthop=nexthop)
        self.expected_subnet.host_routes = self.get_ipv4_host_route_data(num=2)
        self.expected_subnet.host_routes.append(host_route)
    ...
    def test_ipv6_subnet_create_w_multiple_params(self):
        ...
        self.expected_subnet.cidr = cidr
        self.expected_subnet.allocation_pools = self.get_allocation_pools_data(
            cidr=self.expected_subnet.cidr, start_increment=100, ip_range=500,
            interval=50, n=3)
        gateway_ip = self.subnets.behaviors.get_ip(self.expected_subnet.cidr,
                                                   increment=2)
        self.expected_subnet.gateway_ip = gateway_ip
        ...
        nexthop = self.subnets.behaviors.get_ip(self.expected_subnet.cidr)
        host_route = dict(destination='10.0.3.0/24', nexthop=nexthop)
        self.expected_subnet.host_routes = self.get_ipv6_host_route_data(num=2)
        self.expected_subnet.host_routes.append(host_route)
    ...
    def test_ipv4_subnet_dns_nameservers_quota(self):
        ...
        # Setting the expected Subnet and test data params
        self.expected_subnet.name = 'test_sub_create_dns_ipv4_quotas'
        self.expected_subnet.ip_version = 4
        cidr = self.subnets.behaviors.create_ipv4_cidr()
        self.expected_subnet.cidr = cidr
        allocation_pools = self.subnets.behaviors.get_allocation_pools(
        ...
        for _ in range(quota):
            ...
            self.expected_subnet.dns_nameservers.append(dns_nameserver)
            resp = self.subnets.behaviors.create_subnet(
                network_id=self.expected_subnet.network_id,
                name=self.expected_subnet.name,
                ip_version=self.expected_subnet.ip_version,
        ...
        self.expected_subnet.dns_nameservers.append(dns_nameserver)
        resp = self.subnets.behaviors.create_subnet(
            network_id=self.expected_subnet.network_id, ip_version=4,
            dns_nameservers=self.expected_subnet.dns_nameservers,
            raise_exception=False)
    ...
    def test_ipv6_subnet_dns_nameservers_quota(self):
        ...
        # Setting the expected Subnet and test data params
        self.expected_subnet.name = 'test_sub_create_dns_ipv6_quotas'
        self.expected_subnet.ip_version = 6
        cidr = self.subnets.behaviors.create_ipv6_cidr()
        self.expected_subnet.cidr = cidr
        allocation_pools = self.subnets.behaviors.get_allocation_pools(
    ...
    def test_ipv4_subnet_allocation_pools_quota(self):
        ...
        # Setting the expected Subnet and test data params
        self.expected_subnet.name = 'test_sub_create_alloc_pools_ipv4_quotas'
        self.expected_subnet.ip_version = 4
        cidr = self.subnets.behaviors.create_ipv4_cidr()
        self.expected_subnet.cidr = cidr
        self.expected_subnet.allocation_pools = []
        start_increment = 1
        end_increment = 5
        ...
        for _ in range(quota):
            ...
            allocation_pool = self.subnets.behaviors.get_allocation_pools(
                self.expected_subnet.cidr, start_increment=start_increment,
                end_increment=end_increment)
            self.expected_subnet.allocation_pools.append(allocation_pool)
            resp = self.subnets.behaviors.create_subnet(
                network_id=self.expected_subnet.network_id,
                name=self.expected_subnet.name,
                ip_version=self.expected_subnet.ip_version,
        ...
        allocation_pools = self.subnets.behaviors.get_allocation_pools(
            self.expected_subnet.cidr, start_increment=start_increment,
            end_increment=end_increment)
        self.expected_subnet.allocation_pools.append(allocation_pools)
        resp = self.subnets.behaviors.create_subnet(
            network_id=self.expected_subnet.network_id, ip_version=4,
            allocation_pools=self.expected_subnet.allocation_pools,
            raise_exception=False)
    ...
    def test_ipv6_subnet_allocation_pools_quota(self):
        ...
        # Setting the expected Subnet and test data params
        self.expected_subnet.name = 'test_sub_create_alloc_pools_ipv6_quotas'
        self.expected_subnet.ip_version = 6
        cidr = self.subnets.behaviors.create_ipv6_cidr()
        self.expected_subnet.cidr = cidr
        self.expected_subnet.allocation_pools = []
    ...
    def test_ipv4_subnet_host_routes_quota(self):
        ...
        # Setting the expected Subnet and test data params
        self.expected_subnet.name = 'test_sub_create_host_routes_ipv4_quotas'
        self.expected_subnet.ip_version = 4
        cidr = self.subnets.behaviors.create_ipv4_cidr()
        self.expected_subnet.cidr = cidr
        allocation_pools = self.subnets.behaviors.get_allocation_pools(
        ...
        for _ in range(quota):
            ...
            self.expected_subnet.host_routes.extend(host_route)
            resp = self.subnets.behaviors.create_subnet(
                network_id=self.expected_subnet.network_id,
                name=self.expected_subnet.name,
                ip_version=self.expected_subnet.ip_version,
    ...
    def test_ipv6_subnet_host_routes_quota(self):
        ...
        # Setting the expected Subnet and test data params
        self.expected_subnet.name = 'test_sub_create_host_routes_ipv6_quotas'
        self.expected_subnet.ip_version = 6
        cidr = self.subnets.behaviors.create_ipv6_cidr()
        self.expected_subnet.cidr = cidr
        allocation_pools = self.subnets.behaviors.get_allocation_pools(
    ...
    def test_ipv4_subnet_create_w_allocation_pools(self):
        ...
        self.expected_subnet.cidr = cidr
        self.expected_subnet.allocation_pools = self.get_allocation_pools_data(
            cidr=self.expected_subnet.cidr, start_increment=10, ip_range=20,
            interval=2, n=3)
    ...
    def test_ipv6_subnet_create_w_allocation_pools(self):
        ...
        self.expected_subnet.cidr = cidr
        self.expected_subnet.allocation_pools = self.get_allocation_pools_data(
            cidr=self.expected_subnet.cidr, start_increment=100, ip_range=700,
            interval=20, n=3)
