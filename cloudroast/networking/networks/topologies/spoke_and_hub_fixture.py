...
class SpokeAndHubFixture(NetworkingComputeFixture, TopologyFixtureRoutines):
    ...
    def setUp(self):
        ...
        if not self.hub.keys():
            ...
            # (NUM_OF_SPOKES =1), then execute cmd else make sure
    ...
    def _build_spokes(self):
        ...
        # Add the generalize isolated network static route (so any isolated
        # subnet is routed out the local isolated network interface and not the
        # standard default route (public network interface).
    ...
    def _build_hub_router(self, network_spokes):
        ...
        # Wait for VM's public network to come online by pinging the
        # public interface
        attempt = 0
        max_attempts = 10
        hub_available = False
        ...
        # Enable the device to do basic routing
